...
# Manual encoding, tidak menggunakan label encoding untuk kolom2 kategorikal
...
# Fungsi bantuan untuk mendapatkan nilai numerik dari nilai kategorikan yang dipilih
def get_value(val, my_dict):
    for key, value in my_dict.items():
        if val == key:
            return value

# Fungsi bantuan untuk memuat model ke dalam aplikasi
def load_model(model_file):
    loaded_model = joblib.load(open(os.path.join(model_file), 'rb'))
    return loaded_model

def run_ml_app2():
    st.subheader('ML Section - Employee Promotion Prediction')
